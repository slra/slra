...
opt_n.maxiter = 1000; opt_n.disp = 'off'; opt_n.method = 'n'; % for slra with NM method
...
methods = {'#' 'name'      'command'                                                        'line style'
           ...
           4  'pem'          'sys = pem(iddata(yi, ui), ny * l, ''nk'', zeros(1, nu), ''dist'', ''none'', ''ss'', ''can'', ''InitialState'', ''Estimate'', ''LimitError'', 0); x0 = sys.x0; sys = ss(sys.a, sys.b, sys.c, sys.d, -1);' '--r'
           5  'moesp'        'sys = n4sid(iddata(yi, ui), l * ny, ''nk'', zeros(1, nu), ''dist'', ''none'', ''InitialState'', ''Estimate'', ''Focus'', ''Simulation'', ''N4Weight'', ''MOESP''); sys = ss(sys.a, sys.b, sys.c, sys.d, -1);' '--g'
