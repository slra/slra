...
C             (NL*K+1)*K-by-MIN(S,N-P)*K part of this array contains
C             the (P*K+1)-st to (MIN(P+S,N)*K)-th columns of the lower
C             Cholesky factor in banded format.
...
C
     ...
     $          ( .NOT.ISROW .AND. LDT.LT.MAX( 1, ( NL + 1 )*K ) ) )
     $      THEN
         INFO = -9
      ELSE IF ( (      LTRI .AND. LDRB.LT.NL*K + 1 ) .OR.
     $          ( .NOT.LTRI .AND. LDRB.LT.MAX( 1, ( NL + 1 )*K ) ) )
     $      THEN
         INFO = -11
...
C
         DO 90  I = PRE, PRE + STPS - 1
            CALL MB02CU( 'Row', K, K, K, NB, T, LDT, ZERO, 1,
     $                   DWORK(HEAD*K+2), K, RNK, IPVT, DWORK(PDW+1),
     $                   ZERO, DWORK(PDW+3*K+1), LDWORK-PDW-3*K, IERR )
C
            IF ( IERR.NE.0 )  THEN
C
...
C
            ...
            ELSE
               STRUCT = 'N'
            END IF
            CALL MB02CV( 'Row', STRUCT, K, LEN, K, K, NB, -1, ZERO, 1,
     $                   ZERO, 1, DWORK(HEAD*K+2), K, T(1,K+1), LDT,
     $                   ZERO, 1, DWORK((HEAD+K)*K+2), K, DWORK(PDW+1),
     $                   DWORK(PDW+3*K+1), LDWORK-PDW-3*K, IERR )
C
            ...
            ELSE
               STRUCT = 'N'
            END IF
            CALL MB02CV( 'Row', STRUCT, K, LEN2, K, K, NB, -1, ZERO, 1,
     $                   ZERO, 1, DWORK(HEAD*K+2), K, T(1,K+LEN+1), LDT,
     $                   ZERO, 1, DWORK(2), K, DWORK(PDW+1),
     $                   DWORK(PDW+3*K+1), LDWORK-PDW-3*K, IERR )
C
...
C
            CALL MB02CU( 'Column', K, K, K, NB, T, LDT, ZERO, 1,
...
C
            ...
            ELSE
               STRUCT = 'N'
            END IF
            CALL MB02CV( 'Column', STRUCT, K, LEN, K, K, NB, -1, ZERO,
     $                   1, ZERO, 1, DWORK(HEAD+2), LENR, T(K+1,1), LDT,
     $                   ZERO, 1, DWORK(HEAD+K+2), LENR, DWORK(PDW+1),
     $                   DWORK(PDW+3*K+1), LDWORK-PDW-3*K, IERR )
C
            IF ( ( N - I )*K.GE.LENR ) THEN
               STRUCT = TRIU
            ELSE
               STRUCT = 'N'
            END IF
            CALL MB02CV( 'Column', STRUCT, K, LEN2, K, K, NB, -1, ZERO,
     $                   1, ZERO, 1, DWORK(HEAD+2), LENR, T(K+LEN+1,1),
     $                   LDT, ZERO, 1, DWORK(2), LENR, DWORK(PDW+1),
     $                   DWORK(PDW+3*K+1), LDWORK-PDW-3*K, IERR )
C
