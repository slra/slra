...
C             contains the right singular vectors, including null space
C             vectors, if any, of C = [A|B].
C
C     LDC     INTEGER
...
C     .. External Subroutines ..
      EXTERNAL          DCOPY, DGERQF, DGESVD, DLACPY, DLASET, DORMRQ,
     $                  DSWAP, DTRCON, DTRSM, XERBLA
...
C        matrix and exit.
C
         CALL DCOPY( MINMNL-1, DWORK(JWORK+1), 1, DWORK, 1 )
         RETURN
      END IF
...
C
     ...
     $                DWORK(JWORK), LDWORK-JWORK+1, INFO )
         WRKOPT = MAX( 2, WRKOPT, INT( DWORK(JWORK) )+JWORK-1 )
C
