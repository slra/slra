...
C
         DO 20 I = 1, K
            DWORK(PDW) = DLAPY2( DWORK(PDW),
     $                           DNRM2( COL2, A2(I,1), LDA2 ) )
            DWORK(PDW+K) = DWORK(PDW)
            PDW = PDW + 1
   20    CONTINUE
C
         PDW = PDW + K
...
C
               IF ( COL2.GT.1 ) THEN
                  ...
                  ALPHA2  = A2(I,1)
                  A2(I,1) = ONE
                  CALL DLARF( 'Right', K-I, COL2, A2(I,1), LDA2, TAU2,
     $                        A2(I+1,1), LDA2, DWORK(PHV+1) )
                  A2(I,1) = TAU2
...
C
               DO 50  J = I + 1, K
                  PDW   = PDW + 1
                  TEMP  = ONE - ( ABS( A1(J,I) ) / DWORK(PDW) )**2
                  TEMP2 = ONE + P05*TEMP*
     $                          ( DWORK(PDW) / DWORK(K+PDW) )**2
                  IF ( TEMP2.EQ.ONE ) THEN
                     DWORK(PDW) = DLAPY2( DNRM2( K-I, A1(J,I+1), LDA1 ),
     $                            DNRM2( COL2, A1(J,I+1), LDA1 ) )
                     DWORK(K+PDW)   = DWORK(PDW)
                     DWORK(2*K+PDW) = DNRM2( Q, B(J,1), LDB )
                  ELSE
                     IF ( TEMP.GE.ZERO ) THEN
                        DWORK(PDW) =  DWORK(PDW)*SQRT( TEMP )
                     ELSE
                        DWORK(PDW) = -DWORK(PDW)*SQRT( -TEMP )
                     END IF
                  END IF
...
C
                  IF ( LEN.GT.1) THEN
                     ...
                     BETA     = B(J,POS)
                     B(J,POS) = ONE
                     CALL DLARF( 'Right', K-J, LEN, B(J,POS), LDB, TAU2,
     $                           B(J+1,POS), LDB, DWORK(PHV+1) )
                     B(J,POS)  = BETA
                     CS(J*2-1) = TAU2
                  END IF
C
...
C
     ...
     $                           TAU2, A2(1,J+1), LDA2, DWORK )
                     ...
                     ALPHA   = A1(J,J)
                     CALL DLARTG( ALPHA, ALPHA2, C, S, A1(J,J) )
                     CALL DROT( K-J, A1(J,J+1), LDA1, A2(1,J+1), LDA2,
     $                          C, S )
                     CS(2*K+J*2-1) = C
                     CS(2*K+J*2)   = S
  160             CONTINUE
...
C
     ...
     $                        A2(1,J+1), LDA2, DWORK )
               ...
               ALPHA = A1(J,J)
               CALL DLARTG( ALPHA, ALPHA2, C, S, A1(J,J) )
               CALL DROT( K-J, A1(J,J+1), LDA1, A2(1,J+1), LDA2, C, S )
               CS(2*K+J*2-1) = C
               CS(2*K+J*2)   = S
  180       CONTINUE
C
