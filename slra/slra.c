...
int slra(gsl_vector* p, data_struct* s, gsl_matrix* x,
  ...
  if (opt->method < 0 || 
      opt->method > sizeof(submethod_max) / sizeof(submethod_max[0]) || 
      opt->submethod < 0 || 
      opt->submethod > submethod_max[opt->method]) {
  ...
  gsl_multifit_function_fdf fdflm = { 
    ...
    &slra_fdf_reshaped, m * d, n * d, &params};
  gsl_vector *g;
  ...
  case SLRA_OPT_METHOD_QN: /* QN */
    solverqn = gsl_multimin_fdfminimizer_alloc( Tqn[opt->submethod], 
						n * d );
    gsl_multimin_fdfminimizer_set(solverqn, &fdfqn, &x_vec.vector, 
				  stepqn, opt->tol); 
