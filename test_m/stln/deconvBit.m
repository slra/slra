%deconvBit.m *******************************************************************
% this is the same as deconvB.m but with adaptive number of iterations used
% in TLN algorithm (uses toeAit.m)
diary tlntest.out
diary on
format short e
...
noisestd=[ 1.0e-10 1.0e-8 1.0e-6 1.0e-4 5.0e-4 1.0e-3 5.0e-3 1.0e-2 5.0e-2 1.0e-1];
...
disp('                and adaptive number of TLN iterations                  ')
disp('-----------------------------------------------------------------------')
...
disp(' ')
disp('m,n,d,ntest='), disp ([m,n,d,ntest]),
disp(' ')
disp('average solution error obtained by each method and') 
disp('average norm of d smallest sing.values of TLN approx. and')
disp('average number of TLN iterations used')
disp('   noisestd    LS sol.err.  TLS sol.err.  TLN sol.err. TLN noise s.v.  it')
disp('-------------------------------------------------------------------------- ')
...
%
[xtln,E,dB,niter]=toeAit(A,B);
it=it+niter;
% disp(' number of TLN iterations used:'),disp(niter),
ss=svd([A+E B+dB]);
...
% each noise std considered
 fprintf(' %e  %e  %e %e %e    %5.3f\n',noisestd(l),xerr0(l),xerr1(l),xerr2(l),tlnsv,it);
end %loop l=1:ncases
%
